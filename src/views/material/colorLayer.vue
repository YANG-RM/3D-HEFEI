...
viewer.camera.setView({
  ...
  destination: Cesium.Cartesian3.fromDegrees(117.28159,31.867281, 40000),
  orientation: {
    ...
    pitch: Cesium.Math.toRadians(-40), // 向正下方看
    roll: 0.0, // 左右
  },
});
const getJson = async () => {
  const { res } = await getGeojson("/json/hfArea.geojson");
  const { res: pointRes } = await getGeojson("/json/scenePoints.geojson");
